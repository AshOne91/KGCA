...
HRESULT TDirectWrite::SetFont(wchar_t *fontFamily)
{
    HRESULT hr;
...
HRESULT TDirectWrite::CreateDeviceResources( IDXGISurface1*	pSurface)
{
	...
	V_RETURN( m_pD2DFactory->CreateDxgiSurfaceRenderTarget( 
		pSurface, 
		(const D2D1_RENDER_TARGET_PROPERTIES *)&props, 
		&m_pRT ) );
	V_RETURN( m_pRT->CreateSolidColorBrush(		D2D1::ColorF( D2D1::ColorF::Yellow ),
												&m_pBlackBrush ) );
	return S_OK;
}
