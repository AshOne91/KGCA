...
{    
    ...
    TObject m_player;
    m_player.SetPosition(0, 100, 10, 10);
    TQuadtree quadtree;
 ...
 /*   TObject* pObj = new TObject;
    ...
    //m_player.m_vDirection.x = 1.0f;
    //m_player.m_vDirection.y = 0.0f;
    ...
    {   
        ...
        TVector2D vPos(m_player.m_rt.x1, m_player.m_rt.y1);
        // 벡터의 직전의 방정식
       //  결과벡터 = 시작벡터 + 방향벡터* t(거리 내지는 시간) 
        TVector2D vTmp = m_player.m_vDirection * 1.0f;
        vPos = vPos + vTmp;
        m_player.m_rt.x1 = vPos.x;
        m_player.m_rt.y1 = vPos.y;

        if (vPos.x > 100.0f)
        {
            m_player.m_rt.x1 = 100.0f;
            m_player.m_vDirection.x *= -1.0f;
        }
        if (vPos.x < 0.0f)
        {
            m_player.m_rt.x1 = 0.0f;
            m_player.m_vDirection.x *= -1.0f;
        }
        if (vPos.y > 100.0f)
        {
            m_player.m_rt.y1 = 100.0f;
            m_player.m_vDirection.y *= -1.0f;
        }
        if (vPos.y < 0.0f)
        {
            m_player.m_rt.y1 = 0.0f;
            m_player.m_vDirection.y *= -1.0f;
        }
        m_player.SetPosition(m_player.m_rt.x1, m_player.m_rt.y1, 30, 30);
