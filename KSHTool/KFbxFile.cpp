...
void KFbxFile::UpdateSkeleton(ID3D11DeviceContext* pContext,
    float fTime,
    VS_CONSTANT_BONE_BUFFER& cbData)
{
    for (int iBone = 0; iBone < _pObjectList.size(); ++iBone)
    {
        TMatrix matAnimation = _pObjectList[iBone]->Interplate(fTime, _AnimScene);
        cbData.matBone[iBone] = matAnimation;
    }
}

void KFbxFile::UpdateSkinning(ID3D11DeviceContext* pContext,
...
bool KFbxFile::Render(ID3D11DeviceContext* pContext)
{
    pContext->VSSetConstantBuffers(1, 1, &_pAnimBoneCB);
    for (auto obj : _pDrawObjList)
    {
        obj->SetMatrix(&_matWorld, &_matView, &_matProj);
        obj->Render();
    }
    return true;
}

void KFbxFile::SetMatrix(TMatrix* matWorld, TMatrix* matView, TMatrix* matProj)
{
    if (matWorld != nullptr)
    {
        _matWorld = *matWorld;
